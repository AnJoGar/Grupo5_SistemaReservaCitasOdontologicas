...
export class ModalServicioComponent {
  ...
  agregarEditarServicio() {
    ...
    const _Servicio: Servicio = {
      idServicio: this.servicioEditar == null ? 0 : this.servicioEditar.idServicio,
    ...
    if (this.servicioEditar) {
      ...
      this._servicioService.edit(_Servicio).subscribe({
        next: (data) => {
      ...
      this._servicioService.save(_Servicio).subscribe({
        next: (data) => {
